...
/**
 * @author Olaf Delgado
 * @version $Id: TestExtendTo3d.java,v 1.1 2005/07/15 21:08:14 odf Exp $
 */
public class TestExtendTo3d extends TestCase {
    ...
    private List makeSig(final int a, final int b, final int c, final int d) {
        return Arrays.asList(new Object[] { new Integer(a), new Integer(b), new Integer(c), new Integer(d) });
    }
    
    public void testComputeSignatures() {
        final DSymbol ds = new DSymbol(
                "15 1:2 4 5 7 8 10 12 14 15,4 3 6 8 9 11 12 13 15:4 4 8 3");
        final Map sigs = ExtendTo3d.elementSignatures(ds);
    ...
    public void testcomponentMultiplicities() {
        ...
        final Map counts = ExtendTo3d.componentMultiplicities(ds);
    ...
    public void testFirstRepresentatives() {
        List list;
        ExtendTo3d.firstRepresentatives(new DSymbol("0::"));
        list = ExtendTo3d.firstRepresentatives(new DSymbol("1:1,1,1:3,3"));
        assertEquals(1, list.size());
        assertTrue(list.contains(new Integer(1)));
        list = ExtendTo3d.firstRepresentatives(new DSymbol("4:2 4,3 2 4,2 4:4,4"));
        assertEquals(2, list.size());
        assertTrue(list.contains(new Integer(1)));
        assertTrue(list.contains(new Integer(2)));
        list = ExtendTo3d.firstRepresentatives(new DSymbol("4:2 4,4 3,2 4:4,4"));
        assertEquals(1, list.size());
        assertTrue(list.contains(new Integer(1)));
    }
    ...
    public void testSubCanonicalForms() {
        ...
        final List forms = ExtendTo3d.subCanonicalForms(ds);
    ...
    public void testIterator1() {
        ...
        final ExtendTo3d iter = new ExtendTo3d(ds);
    ...
    public void doTest(final DSymbol ds, final int xTotal, final int xGood,
        ...
        while (bases.hasNext()) {
            ...
            for (final Iterator exts = new ExtendTo3d(base); exts.hasNext();) {
